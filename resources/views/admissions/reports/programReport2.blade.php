...
<!--                    <div class="uk-width-medium-1-5">
 ...
 <!-- end filters -->
 <center>Statistics based on First Choice</center>
 <div class="uk-width-xLarge-1-1">
 <div class="md-card">
 <div class="md-card-content">
     ...
     <div class="uk-overflow-container" id='print'>
          <center><span class="uk-text-success uk-text-bold">{!! $programcode->total()!!} Records</span></center>
                 ...
                 <table border="1" class="uk-table uk-table-hover uk-table-align-vertical uk-table-nowrap tablesorter tablesorter-altair" id="ts_pager_filter"> 
                                  <thead>
                                        <tr>
                                            <th rowspan=2 class="uk-width-1-10">PROGRAMS</th>
...
<!--                                            <th>TOTAL </th>-->
                                    ...
                                    <tbody>
                                     ...
                                     <tr>
                                         ...
                                         <td class='md-bg-grey-100 uk-text-small'>{{$sys->allApplicantGender($row->PROGRAMMECODE,"MALE")}}<?php $total3M[] = $sys->allApplicantGender($row->PROGRAMMECODE,"MALE") ?></td>
                                          <td class='md-bg-grey-100 uk-text-small'>{{$sys->allApplicantGender($row->PROGRAMMECODE,"FEMALE")}}<?php $total3F[] = $sys->allApplicantGender($row->PROGRAMMECODE,"FEMALE") ?></td>
                                         <td class='md-bg-cyan-100 uk-text-small'>{{$sys->allApplicants($row->PROGRAMMECODE)}}<?php $total3[] = $sys->allApplicants($row->PROGRAMMECODE) ?></td>
                                          

                                       

                                          
                                            <td class='md-bg-grey-100 uk-text-small'>{{$sys->getApplicantTotalPerProgramGender($row->PROGRAMMECODE,"MALE")}}<?php $total2M[] = $sys->getApplicantTotalPerProgramGender($row->PROGRAMMECODE,"MALE") ?></td>
                                          <td class='md-bg-grey-100 uk-text-small'>{{$sys->getApplicantTotalPerProgramGender($row->PROGRAMMECODE,"FEMALE")}}<?php $total2F[] = $sys->getApplicantTotalPerProgramGender($row->PROGRAMMECODE,"FEMALE") ?></td>
                                         <td class='md-bg-cyan-100 uk-text-small'>{{$sys->getApplicantTotalPerProgram($row->PROGRAMMECODE)}}<?php $total[] = $sys->getApplicantTotalPerProgram($row->PROGRAMMECODE) ?></td>
                                        ...
                                        <tr>
                                            <td>Total Applied</td>
                                            <td colspan=6 class="md-bg-purple-100"><div style="float:right"><?php echo array_sum($total3)  ?></div></td>

                                        </tr>
                                         <tr>
                                            <td>Total Admitted</td>
                                            <td colspan=6 class="md-bg-purple-100"><div style="float:right"><?php echo  array_sum($total)?></div> </td>
