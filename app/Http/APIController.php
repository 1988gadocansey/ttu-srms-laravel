                ...
                if ($bankDetail) {
                    ...
                    if (empty($data)) {
                        ...
                        if ($feeLedger->save()) {
                            ...
                            @StudentModel::where("INDEXNO", $data->INDEXNO)->orWhere("STNO", $data->INDEXNO)->update(array("BILL_OWING" => $owing, "PAID" => $paid,'STATUS'=>'In school'));
                            @$this->updateReceipt();
                            \DB::commit();
                            //return Response::json("Success", "01");
                            header('Content-Type: application/json');
                ...
                if (!empty($data)) {
                    ...
                    if (!empty($data)) {
                        ...
                        if ($feeLedger->save()) {
                            ...
                            @StudentModel::where("INDEXNO", $indexno)->orWhere("STNO", $indexno)->update(array("BILL_OWING" => $owing, "PAID" => $paid,'STATUS'=>'In school'));
                            @$this->updateReceipt();
                            \DB::commit();
                            //return Response::json("Success", "01");
                            header('Content-Type: application/json');
