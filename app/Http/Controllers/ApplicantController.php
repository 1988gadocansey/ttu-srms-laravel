...
//dd($level);
        ...
        if($fee > 0){
            ...
            if ($admit =="admit") {
                ...
                $paid=Models\FeePaymentModel::where("INDEXNO",$student->APPLICATION_NUMBER)
                    ->where("YEAR",$years)
                    ->where("SEMESTER",$sem)
                    ->sum("AMOUNT");
                if(empty($check->STNO)) {
                    ...
                    $query->BILL_OWING = $student->ADMISSION_FEES-$paid;
                    ...
                    Models\StudentModel::where("STNO",$student->APPLICATION_NUMBER)
                        ->update(array("PROGRAMMECODE"=>$program,"LEVEL"=>$level,"YEAR"=>$level,"BILLS"=>$student->ADMISSION_FEES,"BILL_OWING"=>($student->ADMISSION_FEES-$paid)));
